...
const longestString = (arr) => {
  let longestString = arr.reduce((a, b) => (a.length < b.length ? b : a), "");
...
const firstLetters = (arr) => {
  // Solution code here...
  let firstletterConc = arr.map((str) => {
    return str.charAt(0);
  });
  return firstletterConc;
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map((value) => value.replace(/[() -]/g, ""));
};
...
const onlyOddChars = (str) => {
  ...
  let splittedarr = splitted.filter((element, index) => {
    if (index % 2 !== 0) {
      return element;
    }
  });
