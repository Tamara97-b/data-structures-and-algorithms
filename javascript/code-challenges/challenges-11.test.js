...
const count = (target, input) => {
  return input.flatMap((singleArr) => singleArr.filter((num) => num === target))
    .length;
};
...
const totalSum = (input) => {
  // Solution code here...
  return input
    .flatMap((fatedArr) => fatedArr.reduce((acc, pre) => acc + pre, 0))
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map((arr) =>
    arr
      .filter((num) => {
        if (typeof num === "number" && num % 5 === 0) return num;
      })
...
let findMaleAndFemale = (data) => {
  ...
  return data
    .filter(({ gender: findMaleAndFemale }) => {
      if (findMaleAndFemale === "female" || findMaleAndFemale === "male")
...
let findShortest = (data) => {
  // Solution code here...
  const shortest = data.reduce((acc, { height: shortlists }) =>
    shortlists > acc.height ? shortlists : acc
  );
  return data.find(({ height: shortOne }) => shortest === shortOne).name;
};
