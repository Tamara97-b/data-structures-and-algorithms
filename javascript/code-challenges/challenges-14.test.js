...
const sortBy = (property, arr) => {
  return arr.sort((curr, pre) => (curr[property] > pre[property] ? 1 : -1));
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5 
...
const detectTicTacToeWin = (board) => {
  const winningChecker = (board) => {
    for (let i = 0; i < board.length; i++) {
      const xRes = board[i].every((row) => row === "X");
      const yRes = board[i].every((row) => row === "O");
      if (xRes || yRes) return true;
      else return false;
    }
  };
  const reversRowsToColumns = board.map((row, i, arr) => {
    return row.map((_, j) => arr[j][i]);
  });
  const diagonalMaker = board.map((_, i, arr) => {
    if (i === 0) return arr.map((_, j) => [arr[j][i + j]]).flat();
    if (i === 1)
      return arr.map((use, j) => [arr[j][i + use.length - j - 2]]).flat();
    if (i >= 2) return ["", "", ""];
  });

  return (
    winningChecker(board) ||
    winningChecker(diagonalMaker) ||
    winningChecker(reversRowsToColumns)
  );
};
